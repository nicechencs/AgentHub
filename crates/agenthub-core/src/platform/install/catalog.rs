//! Install channel catalog API: GUI/CLI DTOs + allowlist lookups.
//!
//! Package names and script URLs come only from [`InstallContribution`]
//! via [`builtin_install_registry`]. Do not add a second literal allowlist here.
...
fn push_npm(out: &mut Vec<InstallChannelPlan>, agent: AgentId) {
    ...
    out.push(InstallChannelPlan {
        ...
        command: format!("npm i -g --prefix {} {flags}{pkg}", crate::utils::paths::resolve_data_dir(None).map(|d| d.join("npm").display().to_string()).unwrap_or_else(|_| "~/.agenthub/npm".into())),
