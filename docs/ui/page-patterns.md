...
- Chat outline (聊天大纲): with the setting on (default), two or more user messages, and a transcript panel at least 720px wide, a tick rail on the left of the transcript jumps between prompts. Hover magnifies nearby ticks and shows a preview; the current reading turn is highlighted. A jump turns off stick-to-bottom so streaming does not pull the view back. Toggle is Settings → 语言与外观 (`agenthub:chat-outline-enabled`).
