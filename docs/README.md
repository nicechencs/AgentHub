...
| [adapter-design.md](adapter-design.md) | **Adapter 设计与进度**：用户表面 Routes / 本机路由，模块仍叫 Adapter；页面与本机转发只服务本机路由；直接改配置和写进对方认的登录不另开程序；创建绑定走 Hub |
| [bridges-page-redesign.md](bridges-page-redesign.md) | **本机路由页终态 IA**（已落地，表面已改为 Routes / `/routes`）：对象是本机转发进程；侧栏 Routes 有本机路由才出现；单层健康+端口。稳定文档已回写 ui-design / adapter-design / connection-binding-model |
| [adapter-kimi-codex-dogfood.md](adapter-kimi-codex-dogfood.md) | **真机 dogfood**：直连 Kimi→Claude / Anthropic→Pi；本机路由 Kimi→Codex。禁止记录密钥 / prompt / 正文 |
...
| [chat-page-redesign.md](chat-page-redesign.md) | **Chat 工作台已落地**：会话 rail / header 芯片 / 多 Agent 对比 / 过程面板展示层 / 文件拆分；**无独立模型选择器** |
| [capability-matrix.md](capability-matrix.md) | **能力矩阵**：`Capability` 枚举 + 四级状态、现状矩阵、防漂移测试、P0–P4（**v1.0 已落地**） |
| [account-authorization-pool.md](account-authorization-pool.md) | **账号池身份×授权**：同人可多授权并存；去重仅限同一份授权登录；与能力矩阵边界（**已落地**） |
| [adding-an-agent.md](adding-an-agent.md) | 新增 Agent 适配器清单；`accepts[]` 须登记 wire 协议 **和** OAuth 契约槽 |
| [deepseek-harness-integration.md](deepseek-harness-integration.md) | **DeepSeek Harness（`dsh`）**：DeepSeek API 属直接改配置；DSH 不是本机路由。P1–P5 已落地；StructuredStream 仍 Planned |
...
- 「把已有登录接到另一个工具」的领域模型（实现里仍叫票 / 绑定 / 协议图）以 [connection-binding-model.md](connection-binding-model.md) 为唯一真源。读模型 + `plan` / `bind` / `unbind` 已落地；sidecar 迁移未做。实现状态仍以 [agenthub-plan.md §8](agenthub-plan.md#8-当前实现状态以代码与测试为准) 为准。
