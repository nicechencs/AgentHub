...
| `core.chat` | `stop` | 停止成功（`stop ok`）。生成中点停止，以及等待批准时点停止，都记这条。用户停止会先中断传输，传输层可能报 interrupt；这仍是成功停止，搜 `op=stop`，不要当成没记日志 |
| `core.chat` | `stop_fail` | 停止没生效：没有可停的运行时、取消超时等；error。不要把等待批准时的用户停止记成这条 |
