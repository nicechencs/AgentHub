...
| 登录生命周期 | 登录只有一套；Routes 可添加官方登录 / API Key，不得从 Connections 隐藏 | [Product boundaries](product-boundaries.md) |
...
| local_bridge | 当前 Tauri 进程内；默认每个 Agent/surface 一个同口授权池；sidecar 是迁移提案，未部署 | [Adapters and bridges](../concepts/adapters-and-bridges.md) |
...
| 国产 OAuth | 不开 adapter 边，不转 API；国产官方登录不可分享；国产路由只认支持的 API Key | [Product boundaries](product-boundaries.md) |
| API Key 分享 | 所有 API Key 都可分享至连接池并接到其他工具，不按所属 Agent 挡掉 | [Product boundaries](product-boundaries.md) |
...
2. 看到“目标”“未来”“提案”时，不要当作当前能力；尤其是 `agenthub-adapterd` sidecar。本机同口授权池已是默认 Routes 能力，见 [Connections and routing](../concepts/connections-and-routing.md)。
