...
| **J** | 全前端类型名 `AgentKey` | 局部 | `src/**/*.{ts,tsx}` 中类型 `AgentId`（属性名 `agentId` 不动） | `export type AgentId = AgentKey` 别名保留；Rust / IPC / DB 不改 | `rg '\bAgentId\b' src` 仅剩别名定义；过滤 Vitest |
...
代码符号整改（A–J）已完成。本节记录**用户可见**侧栏、分区标题、主按钮与空态 CTA 的定稿；用户以「继续」按推荐默认项落地。依据 [术语表](../reference/terminology.md)。

### 拍板结果

| 项 | 选择 |
| --- | --- |
| 侧栏语言 | **甲**：中文界面侧栏中文化（MCP / 插件等专名保留） |
| Agents 菜单 | **Agent**（单数专名） |
| 令牌子导航 | **入口 Key**（与页内用词统一；英文 Entry keys） |
| 连接池「接入」 | **改为 加入 / 添加** |
| 「官方未提供账号信息」 | **改为 官方未提供登录信息**（测试仍兼容旧串） |
...
| 其余管理项 | 插件 / 总览 / 连接 / 路由 / 设置 | Plugins / Dashboard / Connections / Routes / Settings |
...
- 连接 / 连接池主按钮保持「官方登录」「添加 API Key」「从连接同步」等。
- 「立即切换到此登录」；连接池「已加入 / 添加中」；无用户向「接入」「账号」主按钮。
- 项目「在对话继续」与侧栏「对话」对齐。
- LLM「Token 用量」等计量词不改为入口 Key。

### 实施批次状态

| UI 批次 | 内容 | 状态 |
| --- | --- | --- |
| **U1** | 侧栏 `nav.*` 中文对齐 + en parity + e2e | 已落地 |
| **U2** | Routes「令牌」→「入口 Key」 | 已落地 |
| **U3** | 「账号」「接入」→「登录」「加入/添加」 | 已落地 |
| **U4** | 技能/Connect 提示歧义清理 + 过滤测试 | 已落地 |

兼容：识别旧提示「官方未提供账号信息」「令牌写进」仍映射到新文案。不改业务逻辑与内部符号名。
