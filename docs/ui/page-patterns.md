...
- Approval cards offer Allow / Deny; Always allow when this request includes that option. Codex auto-accepts later command/file prompts only in the current Codex process (typically this turn; not saved). File-change cards title **修改文件** and show the path.
...
- Approval cards: Allow / Deny; Always allow when the request includes that option. Codex in-process remember only (typically this turn; not saved). Grok / Kiro forward the server option and do not auto-approve later prompts. File-change cards show the path.
...
- Runtime confirmation cards use **允许** / **拒绝**. **一直允许** appears only when this request includes that option (Codex command/file prompts always include it; Grok / Kiro only if the ACP request does). Pending option lists are stored with the request so a snapshot or restart can still show the same buttons. After **一直允许**, only Codex auto-accepts later command/file prompts, and only in the current Codex process (typically this turn; not saved). Grok / Kiro forward the server option and do not auto-approve later prompts. Kiro session **完全访问权限** is a separate start-time setting, not this card. Cursor is out of this surface. See [Chat 与 Agent](../concepts/chat-and-agents.md#允许-拒绝-一直允许).
