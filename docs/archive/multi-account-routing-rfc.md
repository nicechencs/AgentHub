...
>
> **归档（2026-08-24）**。产品方向仍以 [../reference/route-compatibility.md](../reference/route-compatibility.md) 的兼容矩阵为准（历史稿 §5.5）。内核（AccountPicker / 请求边界 FSM）已在 `bridge/account.rs`。默认同口授权池已落地，见 [../concepts/connections-and-routing.md](../concepts/connections-and-routing.md)；设计稿见 [unified-loopback-pool.md](unified-loopback-pool.md)。不要把本文重新启用为待办。
>
> 原状态：**设计稿（2026-08-22）**。
