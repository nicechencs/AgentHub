 ...
 * 2. 正文起点：顶栏已含页标题。常规页 pageShell 顶距 12px；全高页列表顶距 12px。第一块不加额外顶距
...
export const pageRhythm = {
  ...
  /**
   * 分栏打开时的列表水平 inset。
   * 左 12 与页头对齐；右内边距收到 8，并把 8px 让到滚动条与分隔条之间。
   * 不要把 12px 右内边距留在 overflow 容器上：那会把空白加在卡片和滚动条之间，
   * 滚动条仍贴着分隔条（分隔条 hit 区还会叠进滚动条）。
   */
  workbenchXSplit: 'pl-3 pr-2 mr-2',
  /** Settings 等：顶栏下的 Tab 行，顶距 12px */
  workbenchHeader: 'shrink-0 px-3 pt-3',
  ...
  /**
   * 全高工作台列表底距。顶距由 WorkbenchSplitPage 与预览列共用 12px，
   * 左右列上下内边对齐。
   */
  workbenchY: 'pb-3',
