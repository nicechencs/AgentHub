import type { AgentProject, AgentSession } from '@/lib/types';

export function seedMockProjects(): AgentProject[] {
  const now = Date.now();
  const h = 3600 * 1000;
  const d = 24 * h;
  return [
    {
      id: 'claude:proj:-C-Users-demo-app',
      agentId: 'claude',
      title: 'app',
      storagePath: 'C:\\Users\\demo\\.claude\\projects\\-C-Users-demo-app',
      actualPath: 'C:\\Users\\demo\\app',
      relativePath: 'projects/-C-Users-demo-app',
      sessionCount: 3,
      messageCount: 66,
      sizeBytes: 192_000,
      updatedAt: new Date(now - 2 * h).toISOString(),
      preview: '修复登录页 token 过期问题，需要检查 refresh 流程…',
    },
    {
      id: 'codex:proj:cwd/C:/Users/demo/codex-work',
      agentId: 'codex',
      title: 'codex-work',
      storagePath: 'C:\\Users\\demo\\.codex\\sessions\\2026\\08',
      actualPath: 'C:\\Users\\demo\\codex-work',
      relativePath: 'cwd/C:/Users/demo/codex-work',
      sessionCount: 1,
      messageCount: 31,
      sizeBytes: 96_000,
      updatedAt: new Date(now - 5 * h).toISOString(),
      preview: '重构 provider 切换逻辑，避免并发写配置…',
    },
    {
      id: 'kimi:proj:__ungrouped__',
      agentId: 'kimi',
      title: '未分类会话',
      storagePath: 'C:\\Users\\demo\\.kimi-code\\sessions',
      actualPath: null,
      relativePath: 'sessions',
      sessionCount: 1,
      messageCount: 9,
      sizeBytes: 22_000,
      updatedAt: new Date(now - 3 * d).toISOString(),
      preview: '请润色 README 中的安装说明…',
    },
    {
      id: 'grok:proj:cwd/C:/Users/demo/app',
      agentId: 'grok',
      title: 'app',
      storagePath: 'C:\\Users\\demo\\.grok\\sessions',
      actualPath: 'C:\\Users\\demo\\app',
      relativePath: 'cwd/C:/Users/demo/app',
      sessionCount: 1,
      messageCount: 11,
      sizeBytes: 36_000,
      updatedAt: new Date(now - 3 * h).toISOString(),
      preview: '继续修登录页…',
    },
    {
      id: 'grok:proj:cwd/C:/Users/demo/perf',
      agentId: 'grok',
      title: 'perf',
      storagePath: 'C:\\Users\\demo\\.grok\\sessions',
      actualPath: 'C:\\Users\\demo\\perf',
      relativePath: 'cwd/C:/Users/demo/perf',
      sessionCount: 1,
      messageCount: 14,
      sizeBytes: 40_000,
      updatedAt: new Date(now - 8 * h).toISOString(),
      preview: '分析冷启动耗时，给出优化建议…',
    },
    {
      id: 'cursor:proj:d-demo-workspace-2026-AgentHub',
      agentId: 'cursor',
      title: 'AgentHub',
      storagePath: 'C:\\Users\\demo\\.cursor\\projects\\d-demo-workspace-2026-AgentHub',
      actualPath: 'D:\\demo\\workspace\\2026\\AgentHub',
      relativePath: 'projects/d-demo-workspace-2026-AgentHub',
      sessionCount: 2,
      messageCount: 6,
      sizeBytes: 26_000,
      updatedAt: new Date(now - 1 * d).toISOString(),
      preview: '帮我改路由页…',
    },
  ];
}

export function seedMockSessions(): AgentSession[] {
  const now = Date.now();
  const h = 3600 * 1000;
  const d = 24 * h;
  return [
    {
      id: 'claude:projects/-C-Users-demo-app/sess-a1.jsonl',
      projectId: 'claude:proj:-C-Users-demo-app',
      agentId: 'claude',
      title: '修复登录页 token 过期问题',
      cwd: 'C:\\Users\\demo\\app',
      path: 'C:\\Users\\demo\\.claude\\projects\\-C-Users-demo-app\\sess-a1.jsonl',
      relativePath: 'projects/-C-Users-demo-app/sess-a1.jsonl',
      sizeBytes: 128_000,
      updatedAt: new Date(now - 2 * h).toISOString(),
      preview: '修复登录页 token 过期问题，需要检查 refresh 流程…',
      messageCount: 42,
      sessionId: 'sess-a1',
    },
    {
      id: 'claude:projects/-C-Users-demo-app/sess-a2.jsonl',
      projectId: 'claude:proj:-C-Users-demo-app',
      agentId: 'claude',
      title: '补充单元测试覆盖率',
      cwd: 'C:\\Users\\demo\\app',
      path: 'C:\\Users\\demo\\.claude\\projects\\-C-Users-demo-app\\sess-a2.jsonl',
      relativePath: 'projects/-C-Users-demo-app/sess-a2.jsonl',
      sizeBytes: 64_000,
      updatedAt: new Date(now - 1 * d).toISOString(),
      preview: '给 auth 模块补测试，目标覆盖率 80%…',
      messageCount: 18,
      sessionId: 'sess-a2',
    },
    {
      id: 'claude:projects/-C-Users-demo-app/sess-missing-cwd.jsonl',
      projectId: 'claude:proj:-C-Users-demo-app',
      agentId: 'claude',
      title: '临时目录对话',
      cwd: '/var/folders/zz/T/.tmp-agenthub-missing/workspace',
      path: 'C:\\Users\\demo\\.claude\\projects\\-C-Users-demo-app\\sess-missing-cwd.jsonl',
      relativePath: 'projects/-C-Users-demo-app/sess-missing-cwd.jsonl',
      sizeBytes: 8_000,
      updatedAt: new Date(now - 2 * d).toISOString(),
      preview: '在已删除的临时目录里继续改登录页…',
      messageCount: 6,
      sessionId: 'sess-missing-cwd',
    },
    {
      id: 'codex:sessions/2026/08/sess-c1.jsonl',
      projectId: 'codex:proj:cwd/C:/Users/demo/codex-work',
      agentId: 'codex',
      title: '重构 provider 切换逻辑',
      cwd: 'C:\\Users\\demo\\codex-work',
      path: 'C:\\Users\\demo\\.codex\\sessions\\2026\\08\\sess-c1.jsonl',
      relativePath: 'sessions/2026/08/sess-c1.jsonl',
      sizeBytes: 96_000,
      updatedAt: new Date(now - 5 * h).toISOString(),
      preview: '重构 provider 切换逻辑，避免并发写配置…',
      messageCount: 31,
      sessionId: 'sess-c1',
    },
    {
      id: 'codex:sessions/2026/08/sess-c1-review.jsonl',
      projectId: 'codex:proj:cwd/C:/Users/demo/codex-work',
      agentId: 'codex',
      title: '# AGENTS.md instructions',
      cwd: 'C:\\Users\\demo\\codex-work',
      path: 'C:\\Users\\demo\\.codex\\sessions\\2026\\08\\sess-c1-review.jsonl',
      relativePath: 'sessions/2026/08/sess-c1-review.jsonl',
      sizeBytes: 12_000,
      updatedAt: new Date(now - 5 * h).toISOString(),
      preview: '工具审批',
      messageCount: 4,
      sessionId: 'sess-c1-review',
      parentSessionId: 'sess-c1',
      threadKind: 'review',
    },
    {
      id: 'codex:sessions/2026/08/sess-c1-spawn.jsonl',
      projectId: 'codex:proj:cwd/C:/Users/demo/codex-work',
      agentId: 'codex',
      title: '重构 provider 切换逻辑',
      cwd: 'C:\\Users\\demo\\codex-work',
      path: 'C:\\Users\\demo\\.codex\\sessions\\2026\\08\\sess-c1-spawn.jsonl',
      relativePath: 'sessions/2026/08/sess-c1-spawn.jsonl',
      sizeBytes: 48_000,
      updatedAt: new Date(now - 5 * h).toISOString(),
      preview: '重构 provider 切换逻辑，避免并发写配置…',
      messageCount: 12,
      sessionId: 'sess-c1',
      parentSessionId: 'sess-c1',
      threadKind: 'subagent',
      agentRole: 'explorer',
    },
    {
      id: 'kimi:sessions/sess-k1.jsonl',
      projectId: 'kimi:proj:__ungrouped__',
      agentId: 'kimi',
      title: '中文文档润色',
      cwd: null,
      path: 'C:\\Users\\demo\\.kimi-code\\sessions\\sess-k1.jsonl',
      relativePath: 'sessions/sess-k1.jsonl',
      sizeBytes: 22_000,
      updatedAt: new Date(now - 3 * d).toISOString(),
      preview: '请润色 README 中的安装说明…',
      messageCount: 9,
      sessionId: 'sess-k1',
    },
    {
      id: 'grok:sessions/C:%2FUsers%2Fdemo%2Fapp/sess-g-app/chat_history.jsonl',
      projectId: 'grok:proj:cwd/C:/Users/demo/app',
      agentId: 'grok',
      title: '登录页续修',
      cwd: 'C:\\Users\\demo\\app',
      path: 'C:\\Users\\demo\\.grok\\sessions\\C:%2FUsers%2Fdemo%2Fapp\\sess-g-app\\chat_history.jsonl',
      relativePath: 'sessions/C:%2FUsers%2Fdemo%2Fapp/sess-g-app/chat_history.jsonl',
      sizeBytes: 36_000,
      updatedAt: new Date(now - 3 * h).toISOString(),
      preview: '继续修登录页…',
      messageCount: 11,
      sessionId: 'sess-g-app',
    },
    {
      id: 'grok:sessions/C:%2FUsers%2Fdemo%2Fperf/sess-g1/chat_history.jsonl',
      projectId: 'grok:proj:cwd/C:/Users/demo/perf',
      agentId: 'grok',
      title: '性能 profiling 建议',
      cwd: 'C:\\Users\\demo\\perf',
      path: 'C:\\Users\\demo\\.grok\\sessions\\C:%2FUsers%2Fdemo%2Fperf\\sess-g1\\chat_history.jsonl',
      relativePath: 'sessions/C:%2FUsers%2Fdemo%2Fperf/sess-g1/chat_history.jsonl',
      sizeBytes: 40_000,
      updatedAt: new Date(now - 8 * h).toISOString(),
      preview: '分析冷启动耗时，给出优化建议…',
      messageCount: 14,
      sessionId: 'sess-g1',
    },
    {
      id: 'cursor:projects/d-demo-workspace-2026-AgentHub/agent-transcripts/0e435bc1-cf05-4a9a-b036-8902f810bd86/0e435bc1-cf05-4a9a-b036-8902f810bd86.jsonl',
      projectId: 'cursor:proj:d-demo-workspace-2026-AgentHub',
      agentId: 'cursor',
      title: '帮我改路由页',
      cwd: 'D:\\demo\\workspace\\2026\\AgentHub',
      path: 'C:\\Users\\demo\\.cursor\\projects\\d-demo-workspace-2026-AgentHub\\agent-transcripts\\0e435bc1-cf05-4a9a-b036-8902f810bd86\\0e435bc1-cf05-4a9a-b036-8902f810bd86.jsonl',
      relativePath:
        'projects/d-demo-workspace-2026-AgentHub/agent-transcripts/0e435bc1-cf05-4a9a-b036-8902f810bd86/0e435bc1-cf05-4a9a-b036-8902f810bd86.jsonl',
      sizeBytes: 18_000,
      updatedAt: new Date(now - 1 * d).toISOString(),
      preview: '帮我改路由页…',
      messageCount: 4,
      sessionId: '0e435bc1-cf05-4a9a-b036-8902f810bd86',
    },
    {
      id: 'cursor:projects/d-demo-workspace-2026-AgentHub/agent-transcripts/0e435bc1-cf05-4a9a-b036-8902f810bd86/subagents/deadbeef-0000-0000-0000-000000000001.jsonl',
      projectId: 'cursor:proj:d-demo-workspace-2026-AgentHub',
      agentId: 'cursor',
      title: '探查项目状态',
      cwd: 'D:\\demo\\workspace\\2026\\AgentHub',
      path: 'C:\\Users\\demo\\.cursor\\projects\\d-demo-workspace-2026-AgentHub\\agent-transcripts\\0e435bc1-cf05-4a9a-b036-8902f810bd86\\subagents\\deadbeef-0000-0000-0000-000000000001.jsonl',
      relativePath:
        'projects/d-demo-workspace-2026-AgentHub/agent-transcripts/0e435bc1-cf05-4a9a-b036-8902f810bd86/subagents/deadbeef-0000-0000-0000-000000000001.jsonl',
      sizeBytes: 8_000,
      updatedAt: new Date(now - 1 * d).toISOString(),
      preview: 'Explore the AgentHub project…',
      messageCount: 2,
      sessionId: 'deadbeef-0000-0000-0000-000000000001',
    },
  ];
}
