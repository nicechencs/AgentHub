...
| O-24 `SidebarContext` | **已处理，本系列冻结。** [`SidebarContext.tsx`](../../src/components/layout/SidebarContext.tsx) 默认值为 `undefined`；`useSidebar()` 缺 Provider 抛 `SidebarProvider is required`。[`App.tsx`](../../src/App.tsx) 根已包裹 `SidebarProvider`。偏好键：`sidebarCollapsed`、`routesNavVisible`（缺省可见）、`pluginsNavVisible`（缺省可见）。 |
