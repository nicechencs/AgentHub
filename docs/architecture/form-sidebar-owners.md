...
| 安装统计 / 更新标记 / Agent 状态 | 底栏：`AgentDot` + `installed/visibleTotal`。catalog 更新：`status.installed && latestVersion && version !== latestVersion` → `ring-warning`。应用更新：`useAppUpdateAvailable()` 只给设置项 `StatusPin`。折叠宽度 48px / 展开默认 224px，外壳 `pageRhythm.shellNav`。 |
