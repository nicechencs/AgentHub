...
| [adapter-design.md](adapter-design.md) | **Adapter 设计与进度**：用户表面 Bridges / 本机桥，模块仍叫 Adapter；页面与本机转发只服务 ③；①② 不另开程序；创建绑定走 Hub |
| [bridges-page-redesign.md](bridges-page-redesign.md) | **本机桥页终态 IA**（已落地）：对象是 loopback 进程；`/bridges`；侧栏 Bridges 有桥才出现；单层健康+端口。稳定文档已回写 ui-design / adapter-design / connection-binding-model |
