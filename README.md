...
- **偏好**：分组显示语言与外观、启动与关闭、侧栏（是否显示路由 / 插件，新安装默认关闭，功能开发中）、路由、技能市场、用量采集
- **本机**：数据目录与日志
...
查看 Claude 和 Grok 已经装好的插件包，可以启用或停用。没有安装按钮；安装仍在各工具里做。
...
- **插件**：现在列出 Claude / Grok 的插件包，已装的可以启用或停用；没有安装按钮。
