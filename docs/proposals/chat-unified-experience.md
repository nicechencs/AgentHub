...
Status: proposed。用户已认可“统一界面、各家分别对接、先 Codex 再 Claude”的方向，并于 2026-09-05 授权使用 GPT‑5.6 Terra / Luna 子 Agent 开始实施。**半面已落地、剩余边界仍在：** B1/B2 的 Codex 持续聊天已落地（见 [B1](../archive/chat-codex-b1.md)、[B2](../archive/chat-codex-b2.md)）；新空 Grok / Kiro 持续聊天已接线（Grok **无**「用于本次」技能）；Claude B3 **首片已接线**：新空会话走 stream-json 持续通道（多轮+图片）；批准卡片与 SDK 宿主仍未做（见 [Claude B3](../archive/chat-claude-b3.md)）。目标设计保持 proposed，不能把已授权实施或半面落地理解为方案已完成，也不能把方案红线改成「Claude 已做」。S0 历史记录见 [接入验证与交接](../archive/chat-codex-s0.md)。
...
主流程固定为：选文件夹 → 说需求 → 看执行过程 → 回答或确认 → 查看结果 → 继续修改。首次使用沿用有效设置，不要求先选模型参数或安装插件。下面是目标设计，不是 Codex 或其他 Agent 当前已支持的清单；界面文案实施时对照 [术语表](../reference/terminology.md)。

| 优先级 | 操作 | 入口和默认行为 | 对应验收 |
| --- | --- | --- | --- |
| 首版基础 | 安装/登录检查、选目录、新建/继续聊天 | 复用 Agents、连接和会话入口；缺少条件时直接给下一步 | A01、A07、A08 |
| 首版基础 | 添加文件/图片 | 输入框“添加”、拖拽或粘贴；发送前显示已选项并允许移除，仅启用支持的类型 | A13 |
| 首版基础 | 示例任务 | 空白会话提供“了解这个项目”“检查问题”等示例；点选只填草稿，由用户发送 | A14 |
| 首版基础 | 过程、问答、确认、补充、停止 | 聊天内操作；原始日志默认折叠 | A02–A06 |
| 首版基础 | 模型、思考强度、模式 | 输入框附近显示模型/模式，思考强度放模型菜单；默认沿用实际有效设置 | A11 |
| 首版基础 | 常用操作菜单 | 点“更多”或输入 `/` 打开同一可搜索菜单，操作不要求记命令 | A14 |
| 首版基础 | 任务结果与失败处理 | 展示完成摘要、修改文件、测试结果和下一步；未知或未运行如实显示 | A16 |
| 首版按能力开放 | 已安装 Skills、插件的使用 | 聊天内展示当前 Agent 可用项，提供用途说明和管理入口；须验证加载/调用 | A15 |
| 首版按能力开放 | 检查当前修改、使用状态 | 接入原生审查或实际用量来源后显示；无数据不推算剩余额度 | A14、A16 |
| 后续扩展 | 插件安装、启停、更新、卸载及 MCP 配置 | 放在现有对应管理页面；先核实已有能力和写入责任，再单独拆任务 | A17 |
| 后续扩展 | 聊天文件导出 | 单独确定格式和内容；首版可复制回复，不等同原生会话迁移 | 独立范围确定后补充 |

“首版按能力开放”要求 S0 给出 supported/unsupported/unknown 及证据，S3 对支持项完成真实接入；未验证或不支持的功能不显示可执行入口，并在验收记录说明原因。基础交互可用不依赖插件安装。完整原生命令列表、原始配置编辑、复杂 Git 操作、多 Agent 同时协作和通用撤销不进入首版。

### 模型与设置生效

- 选项来自当前 Agent、版本和连接实际可用的列表。无有效默认模型时引导选择，不编造“自动推荐”模型。
- 思考强度保留支持的取值，并用短说明解释等待时间可能增加；不保证提高强度就一定更准确。换模型后重新验证兼容选项，不把上一模型的无效值继续发送。
- 模式说明实际行为，尤其是否允许修改文件；“计划”模式与操作批准范围分别表达，切模式不自动扩大权限。
- 默认修改下一次发送的设置，不改变正在执行的任务。若只支持新会话生效，明确提供新建入口；若会影响本机共用配置，遵守前述写入与活动执行限制。
- 保存用户选择与后台确认的有效值。后台拒绝时保留原有效值并提示原因，不留下看似成功的选择；恢复会话时再次核对可用性。

### 常用操作菜单

现行 `/` 菜单只放对话内动作：新建对话、复制最近回复，并按能力加入换模型、思考强度和「用于本次」技能。打开历史、搜索历史、打开设置、打开 Agent、打开连接已有侧栏或顶栏入口，不再进 `/`。选中 Agent 旁不再放重复的 ⋮。菜单与 `/` 搜索共用同一操作定义，支持键盘、中文搜索、无结果和禁用原因；尚未接入的原生命令不伪装为可用项。

每项操作在开发记录中明确来源：AgentHub 本地操作、Agent 原生操作或快捷提问。原生操作必须真实调用对应接口，快捷提问只填入草稿；不把“检查当前修改”的原生审查悄悄替换为普通提示词。选择会话、切设置等操作不作为聊天文字发给模型。菜单只在明确的命令搜索状态拦截输入，普通文字、文件路径和代码中的 `/` 不误触发执行。

### Skills、插件与外部工具

聊天负责使用，现有管理页面负责管理，不另建一套插件安装或配置存储。Skills、插件和 MCP 保留各自含义；插件可能提供 Skills 或工具，并非每个插件都能被“选中发送”。只有支持显式调用的项目才显示“用于本次任务”；自动加载的工具展示可用状态，不能伪造一次调用。

列表需记录来源、稳定 ID、适用 Agent、版本、用途、启用状态及是否需要登录。不能只按显示名称调用，也不能把“已安装”当成“已在当前会话加载”。用户选择后显示已选项并可移除；提交前复核当前 Agent/连接是否仍可用，失效时说明并让用户调整，不静默遗漏或换项。

缺少安装、登录或配置时，跳转对应管理入口并保留聊天草稿。安装/启用完成后刷新真实状态；需要新会话加载时明确说明。插件操作不继承“允许执行一次”的工具批准，也不修改当前运行中的配置。后续管理任务需逐项确认接口与生效范围；不要求新手编辑 JSON 或输入安装命令。

### 完成结果与错误

结果同时显示可读摘要和有证据的修改文件、测试状态。区分测试通过、失败、未运行和结果未知；不能仅因 Agent 回复“完成”就显示测试通过。文件差异归属和修改前确认遵守原有约定。

错误提供与原因相符的操作，例如重新登录、选择目录、继续会话或查看详情，并保留未发送草稿。执行结果不明时不显示会自动重做副作用的“一键重试”。原始日志与技术错误放详情，主要提示使用用户已见过的词。
...
能力不是一个“支持 Chat”的开关。建议包含 `send`、`resume`、`toolApproval`、`userInput`、`steer`、`interrupt`、`modelSelection`、`reasoningEffortSelection`、`modeSelection`、`fileInput`、`imageInput`、`fileChanges`、`usage`、`review`、`skillDiscovery`、`skillInvocation`、`pluginAvailability`，分别附 `supported / unsupported / unknown`、限制、检测版本和证据。界面与后台均检查能力，未知值不默认启用。插件安装/启停/更新/卸载和 MCP 配置分别由管理功能记录能力，不因 Chat 能列出插件就一并启用。

建议共用操作：

| 操作草案 | 必须表达的语义 |
| --- | --- |
| `getCapabilities` / `getOptions` | 返回该安装、版本、连接下可用能力、模型/思考强度/模式组合及生效范围 |
| `listChatActions` / `listAvailableExtensions` | 返回统一操作定义及当前会话可用的扩展 ID、来源、加载状态和调用方式；名称为草案 |
| `startTurn` | conversation、唯一 clientRequestId、文本/附件、设置、显式选择的可调用扩展引用；返回 turnId |
| `executeChatAction` | 校验操作 ID、类型、会话与能力后分发；原生审查若启动任务须复用 run 的确认、停止、事件与保存规则 |
...
| ID | 范围与依赖 | 必须交付 | 完成条件 |
| --- | --- | --- | --- |
| S0 | 接入验证、环境基线；无前置 | Codex 最小真实实验、操作/扩展能力矩阵、模型设置生效范围、接口草案、Claude 登录结论、固定示例工程 | 不改用户工程即可验证会话、确认、问答与停止；菜单和扩展逐项记录原生/本地/快捷提问来源，失败能力有结论 |
...
| S3 | 新手体验与跨平台；依赖 S2 | 模型/思考强度/模式、添加附件、可搜索操作菜单、已安装扩展使用与管理入口、差异和结果展示、三平台验证 | A01–A16 中 Codex 适用项通过；首次任务不要求输入终端命令，不要求安装插件 |
...
S3 补充 T11 模型与生效范围、T12 操作菜单、T13 附件、T14 已安装扩展发现/调用与管理跳转、T15 结果/错误、T16 三平台验收。插件完整管理作为后续独立任务，复用已有管理功能，范围确认后以 A17 验收；不暗含在 T14 中，也不作为 S0–S3 发布前置。

同一迁移、共享 DTO、ChatService 的变更顺序执行；只有范围不重叠且接口已固定时才并行。跨层实施使用实现与独立审查；迁移、锁与确认流程按项目高风险流程执行。文案/样式等局部修改不增加多 Agent 流程。

排期仅供粗估：S0 2–4 个工作日；S1–S2 合计约 2–4 周；S4 在接入方式明确后约 2–4 周。S3 原估 1–2 周只覆盖较小的菜单/文件范围，新增 T11–T16 后须按 S0 能力矩阵重估，不沿用原工期承诺。S5 和后续插件完整管理不设整包承诺。基于一名熟悉项目的开发者并有审查支持，按实际实验结果调整。
...
固定场景放 `src/dev/mocks/`，包括正常执行、确认、提问、取消、失败、断开、重开、模型设置被拒绝、扩展缺失/未加载、菜单搜索和附件失败。脱敏协议 fixture 放各 driver 的测试目录，示例工程也随仓库交付，不引用私人项目；扩展用例使用随仓库可复现的无敏感测试样例，不依赖个人已安装插件。fake 子进程覆盖真正的 stdin/stdout 读写、半行 JSON、乱序、重复和退出，不只测试 mapper。
...
先读本文交接状态：B1/B2 Codex 已落地（docs/archive/chat-codex-b1.md、chat-codex-b2.md）；Grok/Kiro 持续聊天已接线；Claude B3 首片已接线（stream-json）；见 docs/archive/chat-claude-b3.md。S0 只作历史记录。
...
| A11 | 模型/思考强度/模式/连接/目录切换 | 后台实际生效与 UI 一致；不兼容值重新校验，拒绝后保留原有效值；执行中不被共用配置悄悄改变 |
| A12 | 三平台、中文/空格路径、中文输入、键盘操作 | 路径和停止正确；输入法候选确认不误发送；弹层焦点可恢复 |
| A13 | 添加/移除文件图片、拖拽粘贴、不支持类型或大小 | 发送前可核对；限制明确；读取失败或切 Agent 后不静默丢附件，保留草稿 |
| A14 | 点击菜单、`/` 搜索、键盘选择、示例任务 | 同一操作同一行为；示例只填草稿；本地操作不发给模型，原生操作不假装成提示词；普通斜杠不误执行 |
| A15 | Skills/插件可用、缺失、未加载、同名、切 Agent | 依据稳定 ID 和真实加载状态调用；不可用时说明；管理跳转保留草稿；自动加载不伪称显式调用 |
| A16 | 完成/部分完成/失败、测试未运行、用量未知 | 结果有证据，未知不当通过；差异来源明确；错误下一步有效且不自动重发副作用 |
| A17 | 后续插件管理：安装/启停/更新/卸载与 MCP 配置 | 独立任务中验证真实结果和生效范围；失败保留原状态，需重开时说明；不修改活动任务配置，不作为首版门槛 |
...
用户要求减少来回交接；剩余工作按依赖最少合并为以下三大批，每批内部完成实现、独立审查、修复和定向验证，不按单个按钮或单个接口拆回合。批次是工程组织方式，不是对所有 Agent 接口可用性的保证。

| 批次 | 合并范围 | 交付边界 |
| --- | --- | --- |
| B1：Codex 会话基础 | S0 必需实验 + S1 + S2 | 新 Codex 会话的持续回复、真实确认/问答、补充/停止、持久化和重开；保留旧会话兼容，异常不静默切换实现 |
| B2：面向新人的操作 | S3 | 会话模型与思考强度、统一菜单和示例草稿、附件、Skills/插件发现；每项按真实能力启用；完整插件安装管理仍按 A17 单独定范围 |
| B3：其他 Agent 与兼容验收 | S4 + S5 | 原计划：Claude 对接与复用 UI，其他 Agent 的能力调查和可行接入、三平台真实验收。**落地后的边界：** Grok / Kiro 持续聊天已接线；Claude B3 首片（stream-json 多轮+图片）已接线，批准卡片仍未做；ZCode 没有可验证接口时明确边界，不能以终端文字解析假装同等体验 |

B2 的模型选择必须通过 Codex 的会话参数生效，不复用修改本机默认模型的旧入口。模型列表不能证明账号可调用；Skills/插件的已安装列表不能证明本轮已加载。附件需要核对协议输入类型与读取范围，普通文件、图片、音频分别验证。能力不明时保留草稿并说明原因。
...
| S4（B3 Claude） | **首片已接线**（stream-json 多轮+图片）；批准/SDK 仍未做 | 见 [Claude B3](../archive/chat-claude-b3.md)；禁止造假确认 |
...
| Claude 使用方式 | 新空会话 stream-json ChatRuntime；旧会话 print+resume | 本片无批准卡片；不假装 SDK 已验收 |
...
- [Chat 宿主加深与多 Agent 兼容](chat-host-depth.md)：A–I 已合入 `dev`（Options、ACP 目录、`/` 插入草稿、刷新、计划/用量、宿主命令卡片、外部命令行）。Claude 确认通道仍以本页 S4 / B3 为准，不在宿主加深页另起一套。
