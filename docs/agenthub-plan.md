...
| Adapter 规则分析 / 预览 / profile 管理 | ✅ 当前可 bind：Kimi Provider→Claude/Pi reshape、Kimi Provider→Codex bridge、Anthropic Provider/Account→Pi reshape；② preview 已登记：Claude OAuth Account→Pi `anthropic`（`claude-subscription-to-pi-v0`）、Codex OAuth Account（`auth_json` 与非 `auth_json` 同边）→Pi `openai-codex`（`codex-subscription-to-pi-v0`）、Grok/xAI OAuth Account→Pi `xai`（`grok-subscription-to-pi-v0`），均 `config_sync`、`gateKind=preview_only`、`reusePath=native_subscription`、`canApply=false`，bind 未开。Codex 订阅→Claude 保持产品要做、当前 `canApply=false` / `subscription_candidate`。其余见[适配规则矩阵](provider-api-oauth-adaptation.md#4-当前实现矩阵)。写入入口 `bind`/`unbind`，投影不进钱包，见 [connection-binding-model.md](connection-binding-model.md) |
