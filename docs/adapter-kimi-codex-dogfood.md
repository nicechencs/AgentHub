...
### 2. Kimi 会员 Provider → Codex（本机转发 / 本机路由；Chat Completions ≠ Responses）
...
预期：本机转发可 apply；Codex 卡片显示本机路由状态；该 Kimi 行「正用于」含 Codex。

- [ ]

### 3. Claude Anthropic Provider → Pi（直接改配置 / 写槽；不是订阅 OAuth）
...
预期：直接改配置可 apply；Pi 卡片显示直连 / 改配置，**不**显示本机路由；该 Anthropic 行「正用于」含 Pi。
