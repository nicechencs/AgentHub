...
> **现行状态（2026-08-23）：** 侧栏与页标题英文 **Routes**、中文「**路由**」，默认显示；Settings 偏好 `routesNavVisible` 可隐藏侧栏项，隐藏不禁用页面，`/routes` 仍可打开。规范路由 `/routes`。列表/详情显示 **IP+端口**。Connections 现行入口是「**分享 / 路由**」，不是「接到…」。侧栏不再叫「桥与适配」。Settings **四栏** 偏好 / 本机 / 备份 / 关于（备份不并入本机）。托盘菜单 **打开 AgentHub / 打开路由 / 启动路由 / 停止路由 / 退出**。ConnectFlow 芯片 **直连 / 用这份登录 / 本机路由 / 当前不支持**（界面不再标 ①②③）。**现行 chrome 以 [ui-design.md](ui-design.md) 为准。** 下文是历史终态设计文，不要当未完成改名任务。  
...
本文解除 Hub Phase 1 的过渡冻结（[archive/hub-redesign-plan.md](archive/hub-redesign-plan.md) §3.2「不移除 `/adapter`、不改路由结构、侧栏改名『桥与适配』、收掉创建区」）。那是过渡期护栏，不是终态 IA。当前页已被产品判定为：名字错、路由错、版式错。本文给出可实施的终态，而不是再改一次标题。
