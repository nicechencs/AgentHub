...
| 安全 | 只在这台电脑、当前用户；本机转发的登录不进目标工具 | 「没官方书面批准就不能做本机转发」 |

打开写入的条件是工程就绪，不是再讨论「要不要做」。本机转发的非官方通道风险写在预览里，由你确认。
...
目标工具要同时登记「听哪种接口」和「认哪套订阅登录」。订阅登录除了接口，还要带这套登录的身份，否则规划器判不出「写进对方认的登录」。
...
| [cli-and-config.md](cli-and-config.md) | CLI「代理模式」≠ 本机转发 |
...
- 分类在「这份登录 × 那个工具」上，不在登录上。同一把 Kimi Key → Claude 是直接改配置、→ Codex 是本机转发，这是常态。
- 反对「全部走本机转发」：直接改配置和写进对方认的登录本来不必另开程序；强行转发增加进程、语义损失和条款暴露。对听普通 Chat 的工具，应直接改配置。
- 反对「订阅一律本机转发」：Pi 里三家订阅登录已经是「写进对方认的登录」的反例。不能当「不是 API Key」就等于目标不能原生用同一套登录。
- 上一版把「订阅」几乎都写成本机转发，风险是：先造转发、漏掉写进 Pi、界面误显示「需要本机服务」。
