...
> **现行状态（2026-08-23）：** 侧栏英文 **Routes**、中文「**路由**」，永久显示；页标题「**本机路由**」；规范路由 `/routes`。列表/详情显示 **IP+端口**。Connections 现行入口是「**分享 / 路由**」，不是「接到…」。侧栏不再叫「桥与适配」。Settings **四栏** 偏好 / 本机 / 备份 / 关于（备份不并入本机）。托盘菜单 **打开 AgentHub / 打开路由 / 启动路由 / 停止路由 / 退出**。ConnectFlow 芯片 **直连 / 用这份登录 / 本机路由 / 当前不支持**（界面不再标 ①②③）。下文是历史终态设计文，不要当未完成改名任务。  
