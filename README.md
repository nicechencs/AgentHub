...
| **Connections** | 跨工具登录列表。顶部按 Agent 筛选；OAuth 用头像、API Key 用钥匙。行上「分享」（直连 / 写进对方登录）与「路由」（本机转发）。白话说明见 [产品决策](docs/product-decisions.md)；领域模型见 [连接绑定](docs/connection-binding-model.md)。切换前自动备份 live |
| **Routes** | 本机转发运行时（启停、恢复）；侧栏英文 Routes、中文「路由」，永久显示；页标题「本机路由」；列表与详情显示 127.0.0.1 和端口。日常绑定走 Dashboard / Connections；规则见[厂商、API 与 OAuth 规则](docs/provider-api-oauth-adaptation.md) |
| **Skills** | 以 `~/.agents/skills/` 为共享真源，向各 Agent 同步技能 |
| **MCP** | 只读汇总各 Agent 的本机 MCP 配置；顶部按 Agent 筛选。管理与注入仍在规划中 |
