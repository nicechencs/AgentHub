...
- Approval cards offer Allow / Deny; Always allow when this request includes that option. Codex auto-accepts later command/file prompts only in the current Codex process (typically this turn; not saved).
...
- Approval cards: Allow / Deny; Always allow when the request includes that option. Codex in-process remember only (typically this turn; not saved). Grok / Kiro forward the server option and do not auto-approve later prompts.
