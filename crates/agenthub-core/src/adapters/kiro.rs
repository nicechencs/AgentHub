...
//! - subsequent turns: keep the ACP process and `session/prompt`. If the
//!   process exits or model/effort/permissions change, spawn a new ACP
//!   process in the same AgentHub conversation (`session/load` after exit
//!   hangs or dies). Print path still `--resume-id`
