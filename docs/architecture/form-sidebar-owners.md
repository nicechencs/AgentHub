...
| O-24 `SidebarContext` | **已处理，本系列冻结。** [`SidebarContext.tsx`](../../src/components/layout/SidebarContext.tsx) 默认值为 `undefined`；`useSidebar()` 缺 Provider 抛 `SidebarProvider is required`。[`App.tsx`](../../src/App.tsx) 根已包裹 `SidebarProvider`。偏好键：`sidebarCollapsed`、`sidebarAutoCollapseOnRoutes`（缺省打开：点「路由」时折叠）、`routesNavVisible`（缺省隐藏，功能开发中）、`pluginsNavVisible`（缺省隐藏，功能开发中）。 |
| O-23 表单 | [`GenericConfigForm.tsx`](../../src/components/shared/GenericConfigForm.tsx)（约 278 行）是 schema 驱动字段列表。未知 `valueType` 显示不可用，**不**解析 JSON/TOML（那是 [`ConfigEditor`](../../src/components/shared/ConfigEditor.tsx)）。生产调用方只有 [`ProviderEditDialog.tsx`](../../src/components/connections/ProviderEditDialog.tsx) 的 catalog schema 路径。 |
...
| 安装统计 / 更新标记 / Agent 状态 | 窗口底栏：`AgentStatusStrip`（`AgentDot` + `installed/visibleTotal`）。catalog 更新：`status.installed && latestVersion && version !== latestVersion` → `ring-warning`。应用更新：`useAppUpdateAvailable()` 只给设置项 `StatusPin`。折叠宽度 48px / 展开默认 224px，外壳 `pageRhythm.shellNav`。 |
...
```text
pnpm exec vitest run src/components/shared/generic-config-form-map.test.ts src/components/shared/config-field-copy.test.ts src/pages/connections/connections-layout.test.ts src/components/connections/ProviderEditDialog.test.tsx src/pages/providers/remote-models-status.test.ts
pnpm typecheck
pnpm check:docs
```
...
- 源码：`src/components/shared/GenericConfigForm.tsx`、`SecretInput.tsx`、`generic-config-form-map.ts`、`config-field-copy.ts`、`src/components/layout/Sidebar.tsx`、`SidebarContext.tsx`、`sidebar-nav.ts`、`sidebar-agents.ts`、`src/components/connections/ProviderEditDialog.tsx`、`src/lib/agent-visibility.ts`
