...
| 逻辑 | `src/lib/connect-flow/reuse-offer.test.ts` | 真登录常驻「接到…」，只排除自动生成的配置与非登录行；不可行在 ConnectFlow 置灰 + 原因，见 [connection-binding-model.md](connection-binding-model.md) |
