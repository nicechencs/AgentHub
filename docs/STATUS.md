...
- Chat 各家能力与 [Chat 与 Agent](concepts/chat-and-agents.md) 一致：
  ...
  - **停止**：点停止后按钮保持「正在停止」并禁用，直到这一轮真正结束。运行时已经是 `cancelling` 时同样显示「正在停止」。取消请求落空时恢复可点。`error=cancelled` 不把英文 `cancelled` 写在旁边。失败或停止后不出现「重试」或「重试本轮」，可在这场对话里继续发送。
