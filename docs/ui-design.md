...
│ │官方 · v2… │ │xx云 · v0… │ │未安装·点击│ │官方 · v0… │       │
│ └──────────┘ └──────────┘ └──────────┘ └──────────┘       │
│  （N>4 时自动折行；列宽 ≥190px，内容区约 4 列 / 6 列…）   │
...
- Agent 卡片（两行紧凑布局，已装/未装等高）：第一行 logo + 名称 + 右侧认证状态点（绿=已认证/黄=即将过期/红=失效/灰=未配置或未安装）；第二行 meta：已装为 `供应商 · v版本`，未装为引导文案。顺序固定为 `AGENTS` 定义序，不按状态重排。
...
分区：常规（语言/主题/自启动）、**安全**（**凭据脱敏说明**——SecretInput / 不明文回显；**不要求**用户配置主密码或落盘加密，与项目「凭据不加密落盘」决策一致，勿把主密码/keyring 当必填路径）、数据（数据目录只读、**日志级别/保留/打开目录**、用量采集间隔）、**备份**（live 配置快照；自动备份开关已从「数据」移入）、关于（版本/更新）。  
**生效说明**：主题/语言/日志键部分经 core settings 接线；`usageCollectIntervalMin` **已**接前台定时采集（见 §4.6）。`autoStart` / `closeToTray` / `autoBackup` 等仍多为 UI 偏好或占位，**未**全部接系统级能力。换机整库导出未实现，不在「数据」分区展示假入口。
...
- 顶部工具条：**自动备份**（切换即持久化）+ **备份当前 Agent**（随 Tab 变化；未安装禁用）。
...
| `AgentCard` | Dashboard/Agents 页卡片，状态点 + 版本 + 当前配置 |
