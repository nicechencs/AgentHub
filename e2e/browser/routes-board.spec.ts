...
test('routes board follows dashboard layout and local-route usage', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '对话接口' })).toBeVisible({ timeout: 20_000 });
  await expect(page.getByRole('button', { name: '回复接口' })).toBeVisible();
  await expect(page.getByRole('button', { name: '对话补全' })).toBeVisible();
  ...
  await page.getByRole('button', { name: '回复接口' }).click();
