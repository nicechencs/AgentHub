...
本页区分 AgentHub 里三类扩展。用户说的**插件**是各家 `plugin` / `extension` 包，**不是** MCP server。MCP 仍是独立的 `/mcp` 只读页。`/plugins` 列出 Claude / Grok 已装包，并可启用/停用；安装/卸载见 [插件管理提案](../proposals/plugin-management.md)。

## 三类扩展（不要混名）

| 用户界面 | 对象 | 典型厂商入口 | AgentHub 当前 |
|---|---|---|---|
| **插件** | 可安装的 **extension / plugin 包**（常打包 skills、commands、agents、hooks，有时附带 MCP） | Claude `/plugin`、Codex `/plugins`、Grok `plugin`、Pi `pi install` | **列表 + 启用/停用** `/plugins`（Claude / Grok）。无安装按钮，无能力键 |
...
- **插件**（`/plugins`）列出 Claude / Grok 已装包：优先官方 CLI JSON，否则读 live 目录（`~/.claude/plugins/` + `enabledPlugins`，`~/.grok/plugins/`）。已装包可启用/停用；安装仍在各工具里做。设置「显示插件页面」只藏侧栏入口。附带 MCP 只作为包内组件。主栏不展示技能目录、MCP 配置或其他扫描来源。Codex / Pi 仍为 Planned；Cursor / Kimi / WorkBuddy / DSH / ZCode 明确不支持。
...
插件包的 live 状态在各 Agent 自己的目录和 CLI 里。AgentHub 列出已装包，并对已接线的 Claude / Grok 调用官方启用/停用；安装/卸载见 [插件管理提案](../proposals/plugin-management.md)。
