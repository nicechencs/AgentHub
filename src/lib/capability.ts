 ...
 * Cursor is product-locked. Other agents stay available when API Key can enter
 * the pool (`apiKeyAccount` full/partial), even if live `accountSwitch` is
 * unsupported. Missing capability data fails open so loading does not disable
 * every tab.
 */
