...
|---|---|---|---|---|---|---|---|---|---|
| ConfigWrite | Full | Full | Full | Full | Full | Full | Unsupported | Partial | Partial |
| AccountSwitch | Full | Full | Full | Full | Full | Unsupported | Unsupported | Partial | Partial |
| ApiKeyAccount | Full | Partial | Full | Full | Partial | Unsupported | Partial | Full | Full |
...
| ProjectHistory | Full | Full | Full | Full | Full | Full | Partial | Full | Planned |
...
| Usage | Full | Full | Full | Full | Full | Full | Unsupported | Full | Planned |
...
Cursor 的 `ConfigWrite` / `AccountSwitch` 为 Unsupported：没有稳定的本机登录文件可写。界面切换失败时给出中文说明，不静默。**dev 线 store-stamp 默认软隐藏 Cursor Agent**（见 [STATUS](../STATUS.md)）。WorkBuddy / ZCode 本机安装只打开官网，不当成脚本安装失败。ZCode API Key 按目录追加写入 `~/.zcode/v2/config.json` 的一条供应商（官方槽或自定义行），不替换其它条目。
