...
pub(crate) fn write_api_key_auth(path: &Path, api_key: &str) -> Result<()> {
    let body = json!({ (OPENAI_API_KEY_ENV): api_key });
    ...
    match parsed.get(OPENAI_API_KEY_ENV).and_then(|v| v.as_str()) {
        Some(v) if v == api_key => Ok(()),
        _ => Err(AppError::message(
            "provider.verify",
            format!("Codex auth.json {OPENAI_API_KEY_ENV} verification failed after write"),
        )),
    }
}
