...
- Chat 各家能力与 [Chat 与 Agent](concepts/chat-and-agents.md) 一致：
  ...
  - **对话标题**：新建对话先用首条消息提炼的短句。一轮结束后读对方写在自己会话记录里的标题并改用它：Codex（`session_index.jsonl` 的 `thread_name`）、Grok（`summary.json` 的 `generated_title`）、Kiro（`sessions/cli/<id>.json` 的 `title`）、DSH（会话日志的 `session/title` 行）。Claude 没有标题来源，保持首条消息推导。手动改过名字的对话不再被覆盖（不新增「谁起的名字」列，也不做迁移）；连续通道取运行时线程 id，旧会话取 `native_session_id`。AgentHub 不进协议里要标题。
