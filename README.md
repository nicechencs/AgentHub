...
> 跨 Agent 复用的目标模型是「绑定票」。**产品方向**分三路：① API 直连 ② 原生订阅复用 ③ 本机协议桥，见 [产品决策](docs/product-decisions.md)。当前可写入路径仍见[适配规则矩阵](docs/provider-api-oauth-adaptation.md#4-当前实现矩阵)。本地协议桥有实验路径，端到端验收未收口。Cursor 仅支持公开的 Agent CLI 能力，不能作为绑定落点（无 live 写入合同）。
...
- [文档索引](docs/README.md)
- [产品决策（跨 Agent 复用三路）](docs/product-decisions.md)
