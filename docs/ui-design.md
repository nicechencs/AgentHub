...
- **详情展开**：不是字段堆。`sm+` 两栏 **用量**（有配额才出现，QuotaBar 先 7d 再 5h + reset）| **用在哪**（每行一个绑定：左 Agent 名，右「当前使用 / 本机路由运行中 / 本机路由已停止 / 未使用」；空态「还没接到任何工具」）。**更多**默认折叠，只放非重复进阶事实（导入自、人话登录状态如「可续期，尚未验证」、自定义端点 host、API Key / 自定义端点的协议）。卡片头已有的类型 / 来源 / 所属 / 官方账号 / email / 订阅不再出现。页脚：编辑配置或编辑密钥（若可）、右对齐「移入回收站」。
