...
> **以下 Overview 起至文末为实施前原文 / 历史 IA**（2026-08-15）：当时规范路由写 `/bridges`、侧栏写 Bridges、页标题写「本机桥」、并按 PR 1–5 计划实施。阅读时把文中 `/bridges`、Bridges、「本机桥」理解为当时用词，不要当成现行路径或侧栏名。对象三分、单层健康、`partitionLocalBridgeRuntimes` 等 IA 决策仍有效；条件侧栏已取消，侧栏 Routes 永久显示。  
