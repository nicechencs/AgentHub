...
DeepSeek 使用平台签发的 API Key，不是 OAuth。官方 Anthropic 兼容表中存在“忽略”或“不支持”的字段，因此新增规则时必须按目标 Agent 实测文本、流式输出、thinking、工具调用、停止原因和用量，不能只验证请求成功。DeepSeek API 已登记票面；**Claude bind 已开**（experimental `native_endpoint`）。Grok / 订阅仍关；DeepSeek → Pi 未开。
...
| DeepSeek API Provider / Account（preset / extra.provider / 官方 host） | Claude Code | experimental `native_endpoint` | **可实验应用**；写入 `https://api.deepseek.com/anthropic`，凭据只引用 |
...
- GLM Coding Plan、DeepSeek API 已登记票面（speaks 可双协议），classify 只认显式标记；**§6.5 Claude bind 已开**（experimental `native_endpoint`，Provider 与 Account）；Grok / 订阅仍关。GLM/DeepSeek → Pi 未开。
