...
| `-a` / `--agent <id>` | 默认 agent：`claude` \| `codex` \| `kimi` \| `grok` \| `pi` \| `workbuddy` \| `cursor`（真源 `AgentId::ALL`；规划中的 `dsh` 未进枚举） |
...
- 一律小写：`claude` / `codex` / `kimi` / `grok` / `pi` / `workbuddy` / `cursor`（真源 `AgentId::ALL`；规划中的 `dsh` 见 [deepseek-harness-integration.md](deepseek-harness-integration.md)，未进枚举前非法）
