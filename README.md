...
- 管理 Claude Code、Codex、Kimi、Grok、Pi、WorkBuddy、ZCode、DeepSeek Harness、Kiro 和 Cursor Agent 等本机 Agent。Cursor Agent 默认不出现在侧栏和连接页，可在 Agents 里取消隐藏。
...
**MCP** 扫描本机已发现的 MCP，并可探测后写入 Claude / Codex / Grok / Cursor / WorkBuddy。**插件**列出 Claude、Grok 和 Pi 已经装好的包；Claude 和 Grok 可以在这里安装、卸载、启用或停用。Pi 仍只列出已装包。

## 路线图

本机路由、插件以及类似的管理能力会继续补，目前标为**开发中**。

- **插件**：现在列出 Claude / Grok / Pi 的插件包；Claude 和 Grok 可以安装、卸载、启用或停用。Pi 仍只列出。更新仍是提案。
- **MCP**：可扫描，并对 Claude / Codex / Grok / Cursor / WorkBuddy 写入/启用；其余 Agent 与 OAuth 仍不做。
