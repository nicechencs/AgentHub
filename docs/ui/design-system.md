...
The current accent remains indigo, but exposure is deliberately low. Use it for focus, links, checked controls, and the one primary action. Do not use an Agent color as a page background or as a substitute for semantic status colors.
...
| Edge column | `pageRhythm.pageShell` / `workbenchX` with 12px side inset | Tables, lists, dashboards, split workbenches, Routes. When a split pane is open, the scrolling list uses `workbenchXSplit` (12px left; 8px pad + 8px margin on the right) so the scrollbar is not flush against the separator. |
