...
- Streaming process details use a compact summary and an expandable timeline. Commands, stderr, and exit codes stay in a secondary runtime-details disclosure. Codex / Grok turn usage (input / output / cache when the Agent sent it) stays on the reply header so it remains visible after the timeline folds.
...
- Streaming process panel with expandable timeline; Codex / Grok turn usage on the reply header; copy for completed messages only.
