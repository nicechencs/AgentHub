...
- Chat 各家能力与 [Chat 与 Agent](concepts/chat-and-agents.md) 一致：
  ...
  - **过程内用量**：新空 Codex 会话展示 `thread/tokenUsage/updated` 的本轮 last 计数（输入 / 输出 / 缓存）；新空 Grok 会话展示 `turn_completed.usage` 里对方给出的同样字段。只显示协议里有的数字，不估算费用，也不把 Codex 会话累计 `total` 当成这一轮。Kiro 只有对方用同样字段时才会出现，没有单独验收。回复标题行会留下用量；过程面板在结束后默认折叠。
