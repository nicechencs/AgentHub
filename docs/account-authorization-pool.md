...
> **现行状态（dest `02358a3` / 0.2.2）**：loopback import 同一 agent 只 upsert 一个槽；生成投影隐藏。sidecar 未迁。
...
**例外（本机 loopback 桥）**：`127.0.0.1` / `localhost` / `::1` 上的 bridge ticket 不是独立用户授权——bind 每次都会换端口和 bearer。同一 agent 只保留一个 loopback-bridge 槽位，再 import 覆盖（新端口、新 token），不按 token 指纹另开一行。不得与远端 API Key / 官方 OAuth 合并，也不得删 adapter 生成的 projection。生成投影对用户**隐藏**（不进钱包，不是第二张票）。
