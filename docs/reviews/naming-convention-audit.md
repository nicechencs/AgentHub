...
代码符号整改（A–H）已完成。本节只定**用户可见**的侧栏菜单、分区标题、主按钮与空态 CTA 用词；**确认前不改界面文案**。依据 [术语表](../reference/terminology.md)。

### 盘点范围

| 层级 | 真源 | 现状摘要 |
| --- | --- | --- |
| 侧栏分组 | `nav.workspace` / `nav.manage` | 「工作区」「管理」 |
| 侧栏菜单 | `nav.*` | 中英混用：Chat / Agents / Skills / MCP / Projects（英）+ 插件 / 总览 / 连接 / 路由 / 设置（中） |
| Routes 子导航 | `routes.nav.*` | 看板 · 连接池 · 令牌 · 监控 |
| 连接页主按钮 | `connections.*` | 「官方登录」「添加 API Key」「导入授权」「分享至连接池」等，大体已对齐术语表 |
| 连接池主按钮 | `routes.pool.*` | 「官方登录」「添加 API Key」「从连接同步」已对齐；周边仍大量「接入」 |
| 路由页主按钮 | `routes.*` | 「新建路由」「停止」「停止并还原」「写入客户端」「删除路由」 |
| 设置 / 关于 | `settings.*` | 「登录信息」已改；关于标语等仍有「账号」 |

### 需要拍板的原则

1. **侧栏语言**：中文界面是否全部中文化，还是保留 Agent / Skills / MCP 等业界专名英文？
2. **产品面「路由」**：中文固定用「路由」，英文固定 `Routes`；用户文案不出现 bridge / adapter。
3. **「令牌」vs「入口 Key」**：看板/详情已用「入口 Key」；子导航与令牌页标题是否统一为「入口 Key」或「本机令牌」？
4. **「接入」动词**：连接池语境保留「接入」还是改成「添加 / 加入」？
5. **残留「账号」**：用户按钮与提示一律改为「登录」；仅保留「官方未提供账号信息」这类对方系统原文语境。
...
| 当前位置 | 建议中文 | 建议英文 | 说明 |
| --- | --- | --- | --- |
| Chat | 对话 | Chat | 与页面「新建对话」一致 |
| Agents | Agents | Agents | 专名可保留英文，或写作「Agent」 |
| Skills | 技能 | Skills | 与页面标题「技能」一致 |
| MCP | MCP | MCP | 专名保留 |
| Projects | 项目 | Projects | 与页面标题一致 |
| 插件 | 插件 | Plugins | 已一致 |
| 总览 | 总览 | Overview | 英文现为 Dashboard，可改为 Overview 或保持 |
| 连接 | 连接 | Connections | 已一致 |
| 路由 | 路由 | Routes | 已一致；勿改回 Bridges |
| 设置 | 设置 | Settings | 已一致 |

**方案乙（维持现状混用）**：侧栏工作区专名继续英文，管理区继续中文——需书面确认，避免后续反复改。

#### 2. Routes 子导航与页标题

| 当前位置 | 建议 | 理由 |
| --- | --- | --- |
| 看板 | 保持「看板」 | 与「本机转发请求用量」职责匹配 |
| 连接池 | 保持「连接池」 | 术语表产品名 |
| 令牌 | **待选**：`入口 Key` 或 `本机令牌` | 与页内「入口 Key」统一；避免泛称 Token 与上游 API Key 混淆 |
| 监控 | 保持「监控」或改为「活动」 | 英文 Activity；若强调请求轨迹可用「请求」 |
| 「去连接池」 | 保持 | 已清晰 |
...
| 产品面 | 当前 | 建议 | 优先级 |
| --- | --- | --- | --- |
| 连接 | 官方登录 / 添加 API Key / 导入授权 | 保持 | 已对齐 |
| 连接 | 分享至连接池 | 保持 | 术语表固定 |
| 连接 | 「立即切换到此账号」等 | →「立即切换到此登录」 | P1 |
| 连接池 | 官方登录 / 添加 API Key / 从连接同步 | 保持 | 已对齐 |
| 连接池 | 「已接入…」「接入中…」 | **待选**：保持「接入」或 →「已加入 / 添加中」 | P1 |
| 路由 | 新建路由 / 删除路由 / 停止 / 停止并还原 / 写入客户端 | 保持；勿用「建桥」「Adapter」 | — |
| 路由 | 「本机入口」启停 | 保持；与「本机转发」并存时：入口=listener，转发=能力 | 文案注释即可 |
| Agents | 一键安装 / 隐藏 / 卸载… | 保持 | — |
| 技能 | 安装 / 删除到回收站 | 保持；投影动作对用户说「同步到 Agent」而非 project | P2 |
| 对话 | 新建对话 / 停止 / 删除 | 保持 | — |
| 设置 | 各保存/打开目录 | 保持；关于区已用「登录信息」 | — |

#### 4. 明确不改的用户词

- 内部实现名：`Ticket`、`Binding`、`bridge`、`Account`/`Provider` 枚举值。
- 专名：MCP、API Key、Agent（若选方案乙保留英文）。
- 对方系统说法：如路径说明里的官方文件名；「官方未提供账号信息」可保留或改为「官方未提供登录信息」（待选）。

### 建议实施批次（确认后才开工）

| UI 批次 | 内容 | 主要文件 | 验收 |
| --- | --- | --- | --- |
| **U1** | 侧栏 `nav.*` 中英对齐（按拍板的甲/乙） | `zh.ts` / `en.ts`、sidebar 测试、e2e 文案断言 | 中英文 parity；侧栏截图/文案断言 |
| **U2** | Routes 子导航「令牌」等统一 | `routes.nav.*`、令牌页标题、相关测试 | 子导航与页内 Key 用词一致 |
| **U3** | 连接 / 连接池残留「账号」「接入」 | `connections.*`、`routes.pool.*`、onboarding | 无用户向「账号」主按钮；「接入」策略统一 |
| **U4** | 技能投影 / Connect 直连相关提示 | skills / connect-flow 文案 | 用户侧不出现 project/share 歧义 |

### 确认清单（请回复选项）

1. 侧栏：`甲（全中文）` / `乙（维持中英混用）` / `丙（自定义，请注明）`
2. Agents 菜单：`保留 Agents` / `改为 Agent` / `改为 智能体`（不推荐，与术语表不一致）
3. 令牌子导航：`入口 Key` / `本机令牌` / `保持 令牌`
4. 连接池「接入」：`保持` / `改为 加入/添加`
5. 「官方未提供账号信息」：`改为 登录信息` / `保持`

确认后按 U1→U4 改 i18n 与断言测试，不改业务逻辑。
