...
| [Chat 宿主加深与多 Agent 兼容](chat-host-depth.md) | proposed（A–I 已合入 `dev`） | 切片已在 `dev`。本页仍是提案：真窗未宣称对方斜杠命令；F 取消；不拆 enable 白名单。Claude 确认不在本页。现行以 [STATUS](../STATUS.md) 为准。 |
| [可选本机命令护栏](chat-tool-guard.md) | proposed | 占位。本波不实现。 |
...
| [Kiro HTTP / 本机转发](agent-kiro-http.md) | proposed（切片已落地） | HTTP 多轮与本机路由已接线；本机路由 SSE 按上游 event-stream 帧转发（单测；真窗 TTFT 未验）。剩余：企业 IdC/`profileArn`、Chat 打印收齐、官方 REST。 |
