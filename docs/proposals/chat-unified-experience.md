...
Status: proposed。用户已认可“统一界面、各家分别对接、先 Codex 再 Claude”的方向，并继续讨论新手操作范围。当前只更新方案，用户已明确暂不实施；后续开发 Agent 不能仅凭本文任务表开始实现。实现任务、证据和交接状态统一记录在本文，避免依赖某次聊天记录。
...
当前用户要求继续讨论、暂不实施；只有后续明确要求实施时才执行下面开发步骤。
...
| 条目 | 当前状态 | 下一步 |
| --- | --- | --- |
| 方案 | 已补充新手操作范围；继续讨论，暂不实施 | 继续确认范围；收到实施指令后再开始 S0 |
| S0–S5 | 全部未实施、未进行真实验收 | 获得实施指令后从 T01/T02 开始，不先改所有 Agent |
